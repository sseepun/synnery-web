<!DOCTYPE html>
<html lang="th">
<head>
  <?php include_once('include/header.php'); ?>
  <?php include_once('include/style.php'); ?>
</head>
<body class="loading">
  <?php include_once('component/page-loader.php'); ?>
  <?php include_once('include/topnav.php'); ?>
  <?php include_once('include/accessibility.php'); ?>

  <section data-section="1" class="banner-03">
    <div class="pattern style-01">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/01.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-02">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/02.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-03">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/03.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-04">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/04.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-05">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/18.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-06">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/19.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-07">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/20.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-08">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/21.png" alt="Pattern" />
      </div>
    </div>
    <div class="img-bg" style="background-image:url('public/img/bg/banner-20.jpg');"></div>
    <div class="container h-full">
      <div class="text-wrapper" data-aos="fade-up" data-aos-delay="0">
        <div class="wrapper">
          <div class="logo">
            <img src="public/img/logo/logo-20.png" alt="LOGO">
          </div>
          <div class="text">
            <p class="h4 fw-400">MINISTRY OF <span class="color-p fw-700">ENERGY</span></p>
            <h2 class="fw-500 lh-3xs">กระทรวงพลังงาน</h2>
            <a class="btn-link mt-3" href="#">
              <svg class="icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                <path fill="#ec2c23" d="M14.222 6.687a1.5 1.5 0 0 1 0 2.629l-10 5.499A1.5 1.5 0 0 1 2 13.5V2.502a1.5 1.5 0 0 1 2.223-1.314l9.999 5.499Z"/>
              </svg>
              <span class="p sm h-color-p fw-500">รายละเอียดเพิ่มเติม</span>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section data-section="2" class="section-12">
    <div class="container">
      <div class="blocks size-02 flex-column-reverse">
        <div class="block-float left" data-aos="fade-in" data-aos-delay="0">
          <div class="ss-img no-hover ">
            <div class="img-bg" style="background-image:url('public/img/content/83.jpg');"></div>
          </div>
        </div>
        <div class="block">
          <div class="text-container md">
            <h4 class="fw-700 color-p" data-aos="fade-up" data-aos-delay="0">
              OVERVIEW
            </h4> 
            <p data-aos="fade-up" data-aos-delay="150">
              กระทรวงพลังงานมีเพียงอินเตอร์เน็ตเว็บไซต์ (www.energy.go.th)  
              เป็นเครื่องมือในการเผยแพร่ข้อมูลข่าวสาร แต่ในส่วนของสำนักงานปลัดกระทรวงพลังงาน 
              และสำนักงานพลังงานจังหวัด 76 จังหวัด ยังไม่มีเว็บไซต์ของหน่วยงาน จึงจำเป็นต้องการบูรณาการใหม่
            </p>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section class="showcases-tab-container ovf-hidden" id="tab-header">
    <div class="menu-container">
      <?php 
          $tabs = [
          ['title' => 'เว็บไซต์กระทรวง (MINISTRY WEBSITE)'], 
          ['title' => 'เว็บไซต์สำนักงานปลัดกระทรวง (OPS WEBSITE)'], 
          ['title' => 'เว็บไซต์ (WEBSITE)'], 
          ['title' => 'เว็บไซต์ย่อย (MINISITE)'], 
        ]
        ?>
      <div class="tabs">
        <?php foreach($tabs as $i=>$d){?>
          <div class="tab <?= $i==0? 'active': '' ?>" data-tab="tab_<?= $i ?>" data-anchor="#tab-header">
            <div class="wrapper">
              <p class="sm text fw-400"><?= $d['title'] ?></p>
            </div>
          </div>
        <?php }?>
      </div>
    </div>
    <div class="tab-contents">
      <?php foreach($tabs as $i=>$d){?>
        <div class="tab-content <?= $i==0? 'active': '' ?>" data-tab="tab_<?= $i ?>">
          <?php 
            if($i === 0){include('data/showcases/energy.php');} 
            if($i === 1){include('data/showcases/energy.php');} 
            if($i === 2){include('data/showcases/energy.php');} 
            if($i === 3){include('data/showcases/energy.php');} 
          ?>
        </div>
        <?php }?>
    </div>
  </section>

  <section class="section-05 mh-size-02">
    <div class="img-bg" style="background-image:url('public/img/bg/99.jpg');"></div>
  </section>

  <section data-section="9" class="section-padding section-16">
    <div class="container">
      <h4 class="fw-700 color-p text-center" data-aos="fade-up" data-aos-delay="0">
        RECOMMENDED DESIGN
      </h4> 
      <div class="box-content mt-2">
        <h6 class="text-center fw-400" data-aos="fade-up" data-aos-delay="150">
          เว็บไซต์การนิคมอุตสาหกรรมแห่งประเทศไทย (กนอ.)
        </h6>
        <div class="grids">
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100" data-aos="fade-up" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/ieat/screen-01.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100" data-aos="fade-down" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/ieat/screen-02.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section data-section="10" class="section-padding pt-0 section-16">
    <div class="container">
      <div class="box-content">
        <h6 class="text-center fw-400" data-aos="fade-up" data-aos-delay="150">
          เว็บไซต์กรมโยธาธิการและผังเมือง
        </h6>
        <div class="grids">
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100"  data-aos="fade-up" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/dpt/screen-01.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100" data-aos="fade-down" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/dpt/screen-02.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section data-section="11" class="section-padding pt-0 section-16">
    <div class="container">
      <div class="box-content">
        <h6 class="text-center fw-400" data-aos="fade-up" data-aos-delay="150">
          เว็บไซต์กรมอนามัย
        </h6>
        <div class="grids">
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100"  data-aos="fade-up" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/moph/screen-01.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100" data-aos="fade-down" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/moph/screen-02.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section data-section="12" class="section-padding pt-0 section-16">
    <div class="container">
      <div class="box-content">
        <h6 class="text-center fw-400" data-aos="fade-up" data-aos-delay="150">
          เว็บไซต์สำนักงานรับรองมาตรฐานและประเมินคุณภาพการศึกษา
        </h6>
        <div class="grids">
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100"  data-aos="fade-up" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/onesqa/screen-01.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100" data-aos="fade-down" data-aos-delay="300">
                <div class="ss-card ss-card-44">
                  <img class="img" src="public/img/showcase/onesqa/screen-02.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section data-section="13" class="section-padding pt-0 section-16">
    <div class="container">
      <div class="box-content">
        <h6 class="text-center fw-400" data-aos="fade-up" data-aos-delay="150">
          เว็บไซต์สำนักงานคณะกรรมการนโยบายเขตพัฒนาพิเศษภาคตะวันออก
        </h6>
        <div class="grids">
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100" data-aos="fade-up" data-aos-delay="300">
                <div class="ss-card ss-card-44 style-02">
                  <img class="img" src="public/img/showcase/eec/screen-01.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
          <div class="grid lg-50 md-50 sm-100">
            <div class="grids">
              <div class="grid lg-100 md-100 sm-100" data-aos="fade-down" data-aos-delay="300">
                <div class="ss-card ss-card-44 style-02">
                  <img class="img" src="public/img/showcase/eec/screen-02.jpg" alt="Image" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section data-section="14" class="section-12 bg-p">
    <div class="container">
      <div class="blocks">
        <div class="block-float left">
          <div class="ss-img horizontal-01 no-hover">
            <div class="img-bg" style="background-image:url('public/img/content/111.jpg');"></div>
          </div>
        </div>
        <div class="block">
          <div class="text-container md">
            <h4 class="lh-2xs color-white fw-600 lh-sm" data-aos="fade-up" data-aos-delay="0">
              TECHNOLOGY STACK & TOOL 
            </h4>
            <p class="lg color-white mt-2" data-aos="fade-up" data-aos-delay="150">
              สรุปภาพรวมของเทคโนโลยีและเครื่องมือต่างๆ ได้แก่
            </p>
            <div class="gallery-grids jc-center" data-aos="fade-up" data-aos-delay="150">
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M 5.4648438 2 A 1.0001 1.0001 0 0 0 4.46875 3.0898438 L 7.9882812 42.515625 A 1.0001 1.0001 0 0 0 8.7089844 43.388672 L 24.71875 47.960938 A 1.0001 1.0001 0 0 0 25.267578 47.960938 L 41.292969 43.388672 A 1.0001 1.0001 0 0 0 42.013672 42.515625 L 45.53125 3.0898438 A 1.0001 1.0001 0 0 0 44.535156 2 L 5.4648438 2 z M 6.5585938 4 L 43.441406 4 L 40.083984 41.654297 L 24.994141 45.960938 L 9.9199219 41.654297 L 6.5585938 4 z M 19 10 A 1.0001 1.0001 0 0 0 18 11 L 18 29.628906 L 12.267578 28.041016 A 1.0001 1.0001 0 0 0 11 29.003906 L 11 33.949219 A 1.0001 1.0001 0 0 0 11.732422 34.912109 L 22.732422 37.962891 A 1.0001 1.0001 0 0 0 24 37 L 24 11 A 1.0001 1.0001 0 0 0 23 10 L 19 10 z M 27 10 A 1.0001 1.0001 0 0 0 26 11 L 26 28 A 1.0001 1.0001 0 0 0 27.169922 28.986328 L 32.917969 27.996094 L 32.798828 29.767578 L 26.734375 31.435547 A 1.0001 1.0001 0 0 0 26 32.398438 L 26 37 A 1.0001 1.0001 0 0 0 27.265625 37.964844 L 37.265625 35.214844 A 1.0001 1.0001 0 0 0 37.996094 34.335938 L 39.050781 22.085938 A 1.0001 1.0001 0 0 0 37.914062 21.009766 L 32 21.847656 L 32 16 L 38.654297 16 A 1.0001 1.0001 0 0 0 39.650391 15.085938 L 39.996094 11.085938 A 1.0001 1.0001 0 0 0 39 10 L 27 10 z M 20 12 L 22 12 L 22 35.685547 L 13 33.189453 L 13 30.318359 L 18.732422 31.908203 A 1.0001 1.0001 0 0 0 20 30.945312 L 20 12 z M 28 12 L 37.910156 12 L 37.736328 14 L 31 14 A 1.0001 1.0001 0 0 0 30 15 L 30 23 A 1.0001 1.0001 0 0 0 31.140625 23.990234 L 36.951172 23.166016 L 36.0625 33.470703 L 28 35.6875 L 28 33.162109 L 34.015625 31.507812 A 1.0001 1.0001 0 0 0 34.748047 30.611328 L 34.998047 26.861328 A 1.0001 1.0001 0 0 0 33.830078 25.808594 L 28 26.8125 L 28 12 z"></path></svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      Javascript                      
                    </p>
                  </div>
                </div>
              </div>
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M 9 7 L 12 41 L 26 45 L 40 41 C 41 29.667 42 18.333 43 7 L 9 7 z M 11.183594 9 L 40.816406 9 L 38.128906 39.455078 L 26 42.919922 L 13.871094 39.455078 L 11.183594 9 z M 18.550781 15 L 17.589844 27 L 30.580078 27 L 30.169922 32 L 26 32.619141 L 21.880859 32 L 21.699219 30 L 17.839844 30 L 18.230469 35 L 25.990234 37 L 33.759766 35 L 34.75 23 L 22.089844 23 L 22.410156 19 L 30.769531 19 L 31 21 L 34.699219 21 L 34 15 L 18.550781 15 z"></path></svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      HTML5                      
                    </p>
                  </div>
                </div>
              </div>
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" fill-rule="evenodd"><path fill-rule="evenodd" d="M 39 40 L 25 44 L 11 40 L 8 6 L 42 6 C 41 17.332031 40 28.667969 39 40 Z M 39.816406 8 L 10.183594 8 L 12.871094 38.453125 L 25 41.921875 L 37.128906 38.453125 Z M 16.800781 28 L 20.800781 28 L 20.898438 30.5 L 25 31.898438 L 29.101563 30.5 L 29.398438 26 L 20.601563 26 L 20.398438 22 L 29.601563 22 L 29.898438 18 L 16.101563 18 L 15.800781 14 L 34.101563 14 L 33.601563 22 L 32.898438 33.5 L 25 36.101563 L 17.101563 33.5 Z"></path></svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      CSS3                      
                    </p>
                  </div>
                </div>
              </div>
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack h-red" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
                      <path d="M42,37c0,2.762-2.238,5-5,5H11c-2.761,0-5-2.238-5-5V11c0-2.762,2.239-5,5-5h26c2.762,0,5,2.238,5,5 V37z"></path>
                      <path class="text" fill="#ec2c23" d="M33.03,25.6c-0.65-0.9-1.59-1.52-2.8-1.85c0,0,1.02-0.37,1.94-1.75c0.55-0.88,0.83-1.94,0.83-3.18 c0-2.15-0.78-3.8-2.34-4.93C29.1,12.76,27.34,12,24.35,12H15v24h10.43c2.83-0.02,4.96-0.63,6.41-1.8c1.44-1.19,2.16-2.95,2.16-5.3 C34,27.6,33.68,26.5,33.03,25.6z M21,16c0,0,4.17,0,4.25,0c1.52,0,2.75,1.23,2.75,2.75c0,1.52-1.23,2.75-2.75,2.75 c-0.08,0-4.25,0-4.25,0V16z M26,32h-5v-6h5c1.66,0,3,1.34,3,3C29,30.66,27.66,32,26,32z"></path>
                    </svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      Bootstrap                      
                    </p>
                  </div>
                </div>
              </div>
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
                      <path d="M23.084,11.277c-1.633-2.449-1.986-5.722-2.063-7.067c-4.148,0.897-8.269,2.506-8.031,3.691 c0.03,0.149,0.218,0.328,0.53,0.502l-0.488,0.873c-0.596-0.334-0.931-0.719-1.022-1.179c-0.269-1.341,1.25-2.554,4.642-3.709 c2.316-0.789,4.652-1.26,4.751-1.279l0.597-0.12L22,3.6c0,0.042,0.026,4.288,1.916,7.123L23.084,11.277z"></path>
                      <path d="M24.751,43H24.5c-8.192,0-17.309-2.573-18.386-6.879c-0.657-2.63,1.492-5.536,6.214-8.401 l0.52,0.854c-4.249,2.579-6.296,5.172-5.763,7.305c0.935,3.738,9.575,6.068,17.153,6.12c0.901-1.347,5.742-9.26,2.979-19.873 l0.967-0.252c3.149,12.092-3.218,20.837-3.282,20.924L24.751,43z"></path>
                      <path d="M9.931,39.306c-0.539,0-0.806-0.059-0.85-0.07c-0.176-0.043-0.314-0.178-0.362-0.352 c-0.049-0.174,0.001-0.361,0.129-0.488c0.072-0.072,7.197-7.208,8.159-12.978l0.986,0.164c-0.827,4.964-5.715,10.623-7.656,12.707 c1.939-0.111,6.835-1.019,16.234-6.28c-7.335-0.804-8.495-6.676-8.507-6.739l0.983-0.181c0.047,0.246,1.226,6.011,9.244,6.011 c0.003,0,0.005,0,0.008,0l0,0c0.227,0,0.424,0.152,0.482,0.37c0.06,0.218-0.036,0.449-0.231,0.563 C17.315,38.542,11.867,39.305,9.931,39.306z"></path>
                      <path d="M14.524,41.7c-0.207,0-0.395-0.128-0.468-0.325c-0.079-0.211-0.007-0.45,0.177-0.582 c0.034-0.025,1.813-1.338,3.706-4.228c-0.728-0.322-1.465-0.698-2.196-1.137c-0.888-0.533-1.559-1.105-2.06-1.691 c-2.57,0.678-4.942,0.946-7.025,0.769l0.084-0.996c1.876,0.159,4.009-0.063,6.321-0.64c-1.573-2.688-0.129-5.356-0.109-5.392 l0.874,0.487c-0.067,0.122-1.265,2.37,0.249,4.633c2.201-0.632,4.549-1.567,6.979-2.782c0.559-1.835,0.996-3.922,1.225-6.276 c0.016-0.161,0.108-0.304,0.248-0.385s0.311-0.088,0.458-0.021c0.032,0.015,3.264,1.491,5.604,2.454 c0.17,0.07,0.288,0.228,0.307,0.411c0.02,0.183-0.063,0.361-0.216,0.465c-2.289,1.56-4.563,2.913-6.778,4.042 c-0.702,2.225-1.571,4.077-2.459,5.591c3.702,1.383,6.915,1.404,6.956,1.404c0.228,0,0.427,0.154,0.484,0.375 c0.057,0.221-0.042,0.452-0.241,0.563c-4.54,2.522-11.767,3.232-12.072,3.261C14.556,41.699,14.54,41.7,14.524,41.7z M18.909,36.967c-1.04,1.614-2.062,2.773-2.826,3.53c1.998-0.294,5.501-0.938,8.408-2.139 C23.099,38.187,21.084,37.807,18.909,36.967z M14.767,33.431c0.393,0.392,0.883,0.775,1.49,1.14 c0.736,0.442,1.483,0.817,2.22,1.135c0.754-1.264,1.501-2.781,2.142-4.568C18.598,32.1,16.636,32.868,14.767,33.431z M23.202,24.329c-0.205,1.768-0.521,3.381-0.913,4.85c1.66-0.885,3.354-1.896,5.062-3.026 C25.802,25.497,24.099,24.734,23.202,24.329z"></path>
                      <path d="M17.924,10.6c-0.117,0-0.233-0.042-0.325-0.12c-1.61-1.378-3.505-4.182-3.585-4.301 c-0.129-0.191-0.109-0.446,0.046-0.616c0.154-0.171,0.408-0.211,0.608-0.102c0.011,0.003,0.938,0.385,7.217,1.431 c0.181,0.03,0.33,0.156,0.39,0.328c0.061,0.172,0.022,0.364-0.1,0.5c-1.758,1.953-3.979,2.813-4.073,2.848 C18.044,10.589,17.983,10.6,17.924,10.6z M15.647,6.746c0.631,0.849,1.54,1.996,2.372,2.769c0.511-0.233,1.657-0.818,2.744-1.798 C18.18,7.276,16.604,6.962,15.647,6.746z"></path>
                      <path d="M21.843,24.4c-0.068,0-0.137-0.014-0.201-0.042c-0.199-0.088-0.319-0.294-0.296-0.51 c0.292-2.749-3.926-3.852-3.969-3.862c-0.174-0.044-0.312-0.179-0.359-0.352s0.002-0.359,0.129-0.486 c0.207-0.207,5.139-5.098,11.327-7.784c0.173-0.075,0.369-0.047,0.515,0.07c0.145,0.118,0.212,0.307,0.174,0.489 c-1.186,5.744-6.71,12.044-6.944,12.309C22.12,24.341,21.982,24.4,21.843,24.4z M18.455,19.285 c1.184,0.445,3.258,1.475,3.783,3.356c1.449-1.808,4.542-5.973,5.697-9.934C23.548,14.817,19.854,17.999,18.455,19.285z"></path>
                      <path d="M13.079,28.36l-0.475-0.88c1.883-1.015,4.04-2.883,5.807-5.054c-1.504,1.03-2.365,1.735-2.392,1.758 l-0.639-0.77c0.039-0.032,1.764-1.447,4.631-3.22c0.787-1.266,1.392-2.568,1.703-3.816c0.053-0.212,0.099-0.417,0.136-0.615 c-1.925-0.687-3.701-1.094-4.921-1.269c-0.185-0.026-0.339-0.153-0.401-0.328c-0.062-0.175-0.021-0.371,0.104-0.507 c0.085-0.092,2.116-2.268,4.654-3.463c0.197-0.093,0.433-0.047,0.581,0.114c0.067,0.073,1.44,1.615,1.091,4.805 c1.155,0.45,2.345,0.997,3.491,1.648c2.759-1.24,5.892-2.356,9.229-3.03c0.172-0.034,0.363,0.028,0.481,0.168 c0.117,0.14,0.149,0.333,0.083,0.503c-1.3,3.332-4.786,6.891-4.934,7.041c-0.101,0.102-0.239,0.153-0.383,0.148 c-0.143-0.008-0.275-0.076-0.365-0.188c-1.12-1.408-2.584-2.574-4.163-3.523c-2.175,1.004-4.101,2.078-5.684,3.049 C18.693,24.084,15.644,26.979,13.079,28.36z M27.492,17.396c1.29,0.832,2.491,1.81,3.484,2.948 c0.828-0.898,2.815-3.168,3.942-5.422C32.268,15.532,29.76,16.415,27.492,17.396z M22.799,16.122 c-0.033,0.163-0.071,0.33-0.113,0.5c-0.21,0.839-0.544,1.701-0.972,2.561c1.096-0.626,2.309-1.272,3.618-1.898 C24.494,16.841,23.639,16.455,22.799,16.122z M18.048,13.672c1.111,0.218,2.48,0.574,3.941,1.086 c0.152-1.843-0.346-2.972-0.647-3.472C19.966,12.004,18.761,13.014,18.048,13.672z"></path>
                      <path d="M18.05,18.5c0,4.38-3.65,7.86-6.28,10.4c-0.44,0.43-1.93,0.5-1.93,0.5 c0.37-0.38,0.79-0.78,1.24-1.21c2.5-2.42,5.97-5.73,5.97-9.69c0-4.69-1.89-6.54-3.38-8.02c-0.66-0.67-1.22-1.31-1.56-2.09 l0.31-0.13c0.34,0.15,0.73,0.32,1.03,0.45c0.24,0.35,0.56,0.69,0.93,1.06C15.91,11.3,18.05,13.4,18.05,18.5z"></path>
                      <path d="M42.935,19.794c0,0-0.605,0.086-0.775,0.106c-8.76,0.97-17.8,3.49-22.97,5.56 c-1.87,0.75-3.81,1.66-5.58,2.68c-0.01,0.01-0.02,0.01-0.04,0.02C12.53,28.76,10,30,7.95,31.09c3-3.19,8.62-5.65,10.86-6.55 c5.07-2.03,13.78-4.48,22.35-5.53c-1.01-1.18-3.48-3.68-8.34-5.54c-2.84-1.1-7.16-1.72-10.97-2.27c-6.06-0.87-9.51-1.45-9.84-3.1 c-0.07-0.33-0.02-0.66,0.13-0.98c0.33,0.54,0.8,0.92,1.11,1.14c0.15,0.1,0.26,0.16,0.3,0.18l0.01,0.01 c1.42,0.75,5.25,1.3,8.44,1.76c3.86,0.56,8.23,1.19,11.18,2.32c6.87,2.65,9.24,6.44,9.34,6.6 C42.61,19.28,42.935,19.794,42.935,19.794z"></path>
                    </svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      MSSQL                      
                    </p>
                  </div>
                </div>
              </div>
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                      <path d="M 38.011719 4 C 37.574219 3.996094 37.183594 4.273438 37.046875 4.691406 L 32.074219 20 L 17.925781 20 L 12.953125 4.691406 C 12.820313 4.289063 12.449219 4.011719 12.023438 4 C 11.597656 3.992188 11.214844 4.25 11.0625 4.648438 L 5.070313 20.640625 C 5.066406 20.640625 5.066406 20.644531 5.0625 20.648438 L 2.0625 28.648438 C 1.90625 29.070313 2.046875 29.542969 2.414063 29.808594 L 24.40625 45.800781 L 24.410156 45.808594 C 24.414063 45.808594 24.414063 45.808594 24.414063 45.8125 C 24.425781 45.820313 24.441406 45.828125 24.453125 45.835938 C 24.46875 45.84375 24.480469 45.855469 24.496094 45.863281 C 24.5 45.863281 24.5 45.867188 24.503906 45.867188 C 24.503906 45.867188 24.507813 45.871094 24.511719 45.871094 C 24.515625 45.875 24.519531 45.878906 24.527344 45.878906 C 24.53125 45.882813 24.539063 45.886719 24.542969 45.890625 C 24.5625 45.898438 24.585938 45.910156 24.609375 45.917969 C 24.609375 45.917969 24.609375 45.917969 24.609375 45.921875 C 24.632813 45.929688 24.65625 45.9375 24.675781 45.945313 C 24.679688 45.945313 24.679688 45.945313 24.683594 45.949219 C 24.699219 45.953125 24.714844 45.957031 24.734375 45.964844 C 24.742188 45.964844 24.75 45.96875 24.761719 45.96875 C 24.761719 45.972656 24.761719 45.972656 24.761719 45.96875 C 24.78125 45.976563 24.800781 45.980469 24.820313 45.984375 C 24.847656 45.988281 24.871094 45.992188 24.898438 45.996094 C 24.9375 45.996094 24.980469 46 25.019531 46 C 25.058594 45.996094 25.09375 45.996094 25.128906 45.988281 C 25.144531 45.988281 25.15625 45.988281 25.171875 45.984375 C 25.171875 45.984375 25.175781 45.984375 25.179688 45.984375 C 25.1875 45.980469 25.191406 45.980469 25.199219 45.980469 C 25.203125 45.980469 25.207031 45.976563 25.214844 45.976563 C 25.222656 45.972656 25.234375 45.972656 25.242188 45.96875 C 25.257813 45.964844 25.269531 45.960938 25.28125 45.957031 C 25.289063 45.957031 25.292969 45.957031 25.296875 45.953125 C 25.300781 45.953125 25.304688 45.953125 25.308594 45.953125 C 25.324219 45.945313 25.34375 45.9375 25.359375 45.933594 C 25.378906 45.925781 25.394531 45.917969 25.410156 45.910156 C 25.414063 45.910156 25.414063 45.910156 25.417969 45.90625 C 25.421875 45.90625 25.425781 45.90625 25.429688 45.902344 C 25.4375 45.898438 25.445313 45.894531 25.453125 45.890625 C 25.476563 45.878906 25.496094 45.867188 25.515625 45.855469 C 25.523438 45.851563 25.527344 45.847656 25.53125 45.84375 C 25.535156 45.84375 25.539063 45.839844 25.542969 45.839844 C 25.558594 45.828125 25.574219 45.820313 25.589844 45.808594 L 25.597656 45.796875 L 47.589844 29.808594 C 47.953125 29.542969 48.09375 29.070313 47.9375 28.648438 L 44.945313 20.675781 C 44.941406 20.667969 44.9375 20.65625 44.9375 20.648438 L 38.9375 4.648438 C 38.789063 4.261719 38.425781 4.003906 38.011719 4 Z M 8.066406 22 L 16.472656 22 L 22.328125 40.015625 Z M 33.527344 22 L 41.933594 22 L 27.671875 40.015625 Z M 6.3125 23.007813 L 19.6875 39.902344 L 4.203125 28.640625 Z M 43.6875 23.007813 L 45.796875 28.640625 L 30.3125 39.902344 Z"></path>
                    </svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      Gitlab                      
                    </p>
                  </div>
                </div>
              </div>
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                      <path d="M 25 2.2773438 C 24.477218 2.2775288 23.954975 2.3938208 23.472656 2.6269531 L 8.0117188 10.091797 C 7.0499738 10.555895 6.3506462 11.43495 6.1132812 12.476562 L 2.2929688 29.259766 C 2.0568878 30.29908 2.3056798 31.391792 2.96875 32.226562 L 13.671875 45.683594 C 14.338066 46.521569 15.352133 47.009766 16.421875 47.009766 L 33.578125 47.009766 C 34.647867 47.009766 35.661934 46.521569 36.328125 45.683594 L 47.03125 32.228516 L 47.03125 32.226562 C 47.694185 31.39248 47.944721 30.300911 47.708984 29.261719 A 1.0001 1.0001 0 0 0 47.708984 29.259766 L 43.886719 12.476562 A 1.0001 1.0001 0 0 0 43.886719 12.474609 C 43.649412 11.434647 42.950464 10.556829 41.988281 10.091797 L 26.529297 2.6269531 A 1.0001 1.0001 0 0 0 26.527344 2.625 C 26.044986 2.3928828 25.522782 2.2771587 25 2.2773438 z M 25 4.2773438 C 25.224653 4.2774147 25.44878 4.3272999 25.658203 4.4277344 L 41.119141 11.892578 C 41.534958 12.093546 41.834807 12.469885 41.9375 12.919922 L 45.757812 29.703125 C 45.859943 30.153346 45.753012 30.622684 45.466797 30.982422 L 34.763672 44.439453 C 34.475863 44.801477 34.040383 45.009766 33.578125 45.009766 L 16.421875 45.009766 C 15.959617 45.009766 15.526091 44.801477 15.238281 44.439453 L 4.5351562 30.982422 C 4.2482266 30.621193 4.1402683 30.151811 4.2421875 29.703125 L 8.0644531 12.921875 C 8.1670882 12.471488 8.4666043 12.094433 8.8808594 11.894531 A 1.0001 1.0001 0 0 0 8.8808594 11.892578 L 24.341797 4.4277344 C 24.550169 4.3270161 24.775347 4.2772728 25 4.2773438 z M 24.992188 8.484375 C 24.851094 8.4846875 24.707812 8.5128125 24.570312 8.5703125 C 24.020312 8.8103125 23.76 9.45 24 10 C 24.23 10.54 24.309141 11.079141 24.369141 11.619141 C 24.389141 11.899141 24.400625 12.169453 24.390625 12.439453 C 24.420625 12.709453 24.280078 12.98 24.080078 13.25 C 23.870078 13.52 23.850312 13.790547 23.820312 14.060547 C 21.076091 14.329904 18.605618 15.53251 16.716797 17.339844 L 16.699219 17.330078 C 16.469219 17.180078 16.250391 17.029531 15.900391 17.019531 C 15.570391 16.999531 15.270078 16.940234 15.080078 16.740234 C 14.870078 16.580234 14.660703 16.400938 14.470703 16.210938 C 14.080703 15.830937 13.709453 15.420391 13.439453 14.900391 C 13.309453 14.650391 13.069297 14.449375 12.779297 14.359375 C 12.199297 14.199375 11.599688 14.529375 11.429688 15.109375 C 11.259688 15.679375 11.599687 16.289219 12.179688 16.449219 C 12.739688 16.619219 13.209922 16.899219 13.669922 17.199219 C 13.889922 17.349219 14.100547 17.509453 14.310547 17.689453 C 14.540547 17.839453 14.66 18.119453 14.75 18.439453 C 14.815992 18.759984 14.999921 18.945416 15.179688 19.125 C 13.81088 21.074269 13 23.443157 13 26 C 13 26.422964 13.023472 26.840139 13.066406 27.251953 C 12.842073 27.330478 12.62004 27.415663 12.429688 27.640625 C 12.209687 27.890625 11.969219 28.089375 11.699219 28.109375 C 11.439219 28.179375 11.170391 28.229766 10.900391 28.259766 C 10.360391 28.329766 9.8102344 28.37 9.2402344 28.25 C 8.9602344 28.2 8.6503906 28.259453 8.4003906 28.439453 C 7.9103906 28.779453 7.800625 29.459219 8.140625 29.949219 C 8.490625 30.439219 9.1701562 30.560937 9.6601562 30.210938 C 10.140156 29.870938 10.649922 29.679766 11.169922 29.509766 C 11.429922 29.429766 11.690938 29.360547 11.960938 29.310547 C 12.220937 29.220547 12.510313 29.299688 12.820312 29.429688 C 13.077895 29.562633 13.305954 29.552045 13.53125 29.529297 C 14.411477 32.383087 16.336155 34.773926 18.849609 36.283203 C 18.763733 36.523485 18.683169 36.773884 18.759766 37.089844 C 18.819766 37.419844 18.829453 37.730938 18.689453 37.960938 C 18.579453 38.210938 18.460078 38.449453 18.330078 38.689453 C 18.050078 39.149453 17.750547 39.609766 17.310547 40.009766 C 17.100547 40.199766 16.959219 40.469297 16.949219 40.779297 C 16.929219 41.379297 17.4 41.880391 18 41.900391 C 18.6 41.920391 19.110859 41.449609 19.130859 40.849609 C 19.140859 40.259609 19.300469 39.740703 19.480469 39.220703 C 19.570469 38.970703 19.680781 38.720469 19.800781 38.480469 C 19.890781 38.220469 20.129922 38.029141 20.419922 37.869141 C 20.708796 37.720044 20.838739 37.509224 20.96875 37.287109 C 22.230991 37.739556 23.5843 38 25 38 C 26.439867 38 27.815298 37.730979 29.095703 37.263672 C 29.231135 37.494815 29.370326 37.714564 29.660156 37.869141 C 29.950156 38.039141 30.189297 38.220469 30.279297 38.480469 C 30.399297 38.730469 30.499844 38.980469 30.589844 39.230469 C 30.769844 39.750469 30.919453 40.269375 30.939453 40.859375 C 30.939453 41.149375 31.059062 41.430625 31.289062 41.640625 C 31.739062 42.040625 32.420078 42.010312 32.830078 41.570312 C 33.230078 41.120312 33.199766 40.439297 32.759766 40.029297 C 32.319766 39.639297 32.02 39.169219 31.75 38.699219 C 31.62 38.469219 31.500625 38.220469 31.390625 37.980469 C 31.250625 37.740469 31.260078 37.439375 31.330078 37.109375 C 31.410078 36.769375 31.320234 36.519766 31.240234 36.259766 L 31.232422 36.234375 C 33.725863 34.710051 35.625253 32.311926 36.486328 29.458984 L 36.519531 29.460938 C 36.799531 29.480937 37.069141 29.509844 37.369141 29.339844 C 37.669141 29.199844 37.970469 29.119219 38.230469 29.199219 C 38.490469 29.239219 38.759531 29.299141 39.019531 29.369141 C 39.539531 29.519141 40.060781 29.699297 40.550781 30.029297 C 40.790781 30.179297 41.090625 30.249687 41.390625 30.179688 C 41.980625 30.049688 42.350703 29.470625 42.220703 28.890625 C 42.090703 28.300625 41.509922 27.930547 40.919922 28.060547 C 40.339922 28.190547 39.8 28.169141 39.25 28.119141 C 38.98 28.089141 38.719219 28.050234 38.449219 27.990234 C 38.179219 27.970234 37.940938 27.790781 37.710938 27.550781 C 37.480938 27.280781 37.220937 27.210859 36.960938 27.130859 L 36.943359 27.125 C 36.978072 26.754075 37 26.379828 37 26 C 37 23.520714 36.243083 21.213679 34.949219 19.298828 C 35.13888 19.099219 35.330515 18.909707 35.400391 18.570312 C 35.490391 18.250312 35.609844 17.970312 35.839844 17.820312 C 36.039844 17.640313 36.260469 17.480312 36.480469 17.320312 C 36.930469 17.020312 37.400937 16.730312 37.960938 16.570312 C 38.240938 16.490313 38.479141 16.299531 38.619141 16.019531 C 38.899141 15.489531 38.690391 14.830547 38.150391 14.560547 C 37.620391 14.280547 36.959453 14.489297 36.689453 15.029297 C 36.419453 15.549297 36.049922 15.959844 35.669922 16.339844 C 35.469922 16.529844 35.270547 16.710859 35.060547 16.880859 C 34.870547 17.080859 34.570234 17.140156 34.240234 17.160156 C 33.890234 17.170156 33.669453 17.330469 33.439453 17.480469 C 31.532566 15.591338 28.997515 14.337129 26.179688 14.060547 C 26.149687 13.790547 26.129922 13.52 25.919922 13.25 C 25.719922 12.98 25.579375 12.709453 25.609375 12.439453 C 25.599375 12.169453 25.610859 11.899141 25.630859 11.619141 C 25.690859 11.079141 25.77 10.54 26 10 C 26.11 9.74 26.12 9.430625 26 9.140625 C 25.82 8.728125 25.415469 8.4834375 24.992188 8.484375 z M 23.558594 16.115234 L 23.509766 16.5 C 23.369766 17.58 23.269922 18.67 23.169922 19.75 C 23.109922 20.32 23.059531 20.900703 23.019531 21.470703 C 22.529531 21.120703 22.039531 20.749922 21.519531 20.419922 C 20.619531 19.809922 19.719062 19.199141 18.789062 18.619141 L 18.474609 18.423828 C 19.881084 17.211935 21.629049 16.393039 23.558594 16.115234 z M 26.441406 16.115234 C 28.449951 16.404413 30.258788 17.284 31.693359 18.578125 L 31.359375 18.789062 C 30.439375 19.379062 29.550391 19.989609 28.650391 20.599609 C 28.290391 20.839609 27.939609 21.089844 27.599609 21.339844 C 27.329609 21.529844 26.959453 21.359297 26.939453 21.029297 C 26.909453 20.599297 26.870078 20.18 26.830078 19.75 C 26.730078 18.67 26.630234 17.58 26.490234 16.5 L 26.441406 16.115234 z M 16.582031 20.607422 L 16.900391 20.939453 C 17.660391 21.729453 18.430937 22.489766 19.210938 23.259766 C 19.540937 23.589766 19.870937 23.900937 20.210938 24.210938 C 20.450937 24.430937 20.360781 24.820156 20.050781 24.910156 C 19.560781 25.060156 19.070078 25.199375 18.580078 25.359375 C 17.550078 25.699375 16.510469 26.020625 15.480469 26.390625 L 15.027344 26.548828 C 15.017439 26.365855 15 26.185504 15 26 C 15 24.009707 15.585092 22.163224 16.582031 20.607422 z M 33.537109 20.804688 C 34.458955 22.319179 35 24.0924 35 26 C 35 26.167032 34.98265 26.32915 34.974609 26.494141 L 34.619141 26.380859 C 33.579141 26.040859 32.540234 25.749219 31.490234 25.449219 C 30.810234 25.249219 30.129219 25.070391 29.449219 24.900391 C 29.959219 24.410391 30.490469 23.929687 30.980469 23.429688 C 31.750469 22.659688 32.519531 21.889844 33.269531 21.089844 L 33.537109 20.804688 z M 24.380859 24 L 25.599609 24 C 25.789609 24 25.970313 24.090469 26.070312 24.230469 L 26.849609 25.210938 C 26.959609 25.340937 27.000703 25.529219 26.970703 25.699219 L 26.689453 26.910156 C 26.659453 27.100156 26.529141 27.249844 26.369141 27.339844 L 25.25 27.859375 C 25.09 27.949375 24.890469 27.949375 24.730469 27.859375 L 23.609375 27.339844 C 23.449375 27.249844 23.309062 27.100156 23.289062 26.910156 L 23.009766 25.699219 C 22.979766 25.529219 23.020859 25.340938 23.130859 25.210938 L 23.910156 24.230469 C 24.010156 24.090469 24.190859 24 24.380859 24 z M 20.769531 28.769531 C 21.079531 28.719531 21.330938 29.030312 21.210938 29.320312 C 21.020937 29.820312 20.839922 30.310547 20.669922 30.810547 C 20.299922 31.840547 19.939609 32.860391 19.599609 33.900391 L 19.462891 34.330078 C 17.683095 33.146647 16.31612 31.405254 15.587891 29.355469 L 16.119141 29.310547 C 17.209141 29.210547 18.279141 29.079219 19.369141 28.949219 C 19.839141 28.899219 20.299531 28.829531 20.769531 28.769531 z M 29.410156 28.910156 C 29.880156 28.960156 30.340547 29.010781 30.810547 29.050781 C 31.900547 29.150781 32.980312 29.250312 34.070312 29.320312 L 34.417969 29.339844 C 33.702862 31.363494 32.36593 33.088437 30.623047 34.273438 L 30.509766 33.910156 C 30.179766 32.870156 29.820703 31.840312 29.470703 30.820312 C 29.310703 30.360312 29.150469 29.910938 28.980469 29.460938 C 28.870469 29.170938 29.110156 28.870156 29.410156 28.910156 z M 25.074219 30.882812 C 25.216719 30.882812 25.359453 30.954609 25.439453 31.099609 C 25.659453 31.519609 25.890859 31.929844 26.130859 32.339844 C 26.670859 33.279844 27.209063 34.230391 27.789062 35.150391 L 28.025391 35.533203 C 27.071252 35.83443 26.055471 36 25 36 C 23.976312 36 22.989581 35.844572 22.060547 35.560547 L 22.310547 35.160156 C 22.890547 34.240156 23.440234 33.299375 23.990234 32.359375 C 24.240234 31.949375 24.480937 31.529609 24.710938 31.099609 C 24.790937 30.954609 24.931719 30.882812 25.074219 30.882812 z"></path>
                    </svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      Kubernetes                      
                    </p>
                  </div>
                </div>
              </div>
              <div class="grid xl-25 lg-25 md-1-3 sm-20 xs-1-3 pt-3">
                <div class="ss-card ss-card-10 style-03">
                  <div class="icon">
                    <svg class="tech-stack" version="1.0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96.000000 96.000000" preserveAspectRatio="xMidYMid meet"> 
                      <g transform="translate(0.000000,96.000000) scale(0.100000,-0.100000)" stroke="none">
                        <path d="M403 945 c-122 -33 -242 -125 -297 -226 l-28 -50 364 3 363 3 44 25c73 41 74 51 12 114 -121 122 -300 173 -458 131z"></path>
                        <path d="M364 623 c-276 -3 -303 -5 -309 -21 -14 -35 -17 -159 -6 -214 l12-57 342 6 c376 6 379 7 418 69 39 64 15 165 -48 197 -41 21 -77 23 -409 20z"></path><path d="M90 277 c0 -24 55 -101 107 -149 192 -178 486 -168 668 23 31 32 5463 50 69 -3 5 -27 22 -53 37 l-47 28 -362 3 c-300 2 -363 1 -363 -11z"></path>
                      </g>
                    </svg>                    
                  </div>
                  <div class="text-container mt-2">
                    <p class="xxs title text-center color-white fw-400">
                      Elasticsearch                      
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section data-section="15" class="section-17 bg-08 section-padding">
    <div class="container" style="z-index:3;">
      <h3 class="lg text-center color-01 fw-800" data-aos="fade-up" data-aos-delay="0">
        TRAINING
      </h3>
      <div class="grids">
        <div class="grid lg-50 md-50 sm-100 mt-0" data-aos="fade-up" data-aos-delay="150">
          <div class="d-flex flex-column jc-center height-full">
            <p class="title color-white fw-300">
              บริษัทฯ ได้จัดฝึกอบรมการใช้งานหลักสูตรสำหรับดูแลระบบ และผู้ใช้งานทั่วไปให้กับเจ้าหน้าที่ทั้งหมด 
              ไม่น้อยกว่า 180 คน  ผ่านระบบ VDO Conference เนื่องจากอยู่ในสถานการณ์การแพร่ระบาดของ COVID-19
            </p>
          </div>
        </div>
        <div class="grid lg-50 md-50 sm-100" data-aos="fade-up" data-aos-delay="150">
          <div class="ss-card ss-card-42">
            <div class="ss-img">
              <div class="img-bg" style="background-image:url('public/img/content/112.jpg');"></div>
            </div>
          </div>
        </div>
      </div>
    </div>   
    <div class="pattern style-01">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/05.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-02">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/08.png" alt="Pattern" />
      </div>
    </div>   
    <div class="pattern style-03">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/12.png" alt="Pattern" />
      </div>
    </div>       
    <div class="pattern style-04">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/07.png" alt="Pattern" />
      </div>
    </div>     
    <div class="pattern style-05">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/05.png" alt="Pattern" />
      </div>
    </div>  
    <div class="pattern style-06">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/13.png" alt="Pattern" />
      </div>
    </div>  
    <div class="pattern style-07">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/14.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-08">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/15.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-09">
      <div class="wrapper" value=".5">
        <img src="public/img/patterns/16.png" alt="Pattern" />
      </div>
    </div>
    <div class="pattern style-10">
      <div class="wrapper" value="-.5">
        <img src="public/img/patterns/17.png" alt="Pattern" />
      </div>
    </div>
  </section>

  <?php $withFooterInfo=true; include_once('include/footer.php'); ?>
  <?php include_once('include/script.php'); ?>

</body>
</html>